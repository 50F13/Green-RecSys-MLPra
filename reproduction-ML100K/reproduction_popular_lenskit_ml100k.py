...
from lenskit.basic import bias, BiasModel, PopScorer
...
def main():
    ...
    for fraction in downsample_fractions:
        ...
        # create model and pipeline for popularity
        model_popular = PopScorer()
        pipe_popular = topn_pipeline(model_popular)
        
        # run the pipeline with training and validation data
        mean_ndcg = evaluate_with_ndcg(pipe_popular, downsampled_train_data, validation_data)

        # print the results
        print(f"nDCG mean for validation set: {mean_ndcg:.4f}")

        # build the final model and pipeline for the test data
        final_model = PopScorer()
    ...
    for result in results_downsampling:
        print(f"Downsample Percentage: {result['Fraction']*100}% Final Mean nDCG: {result['Final Mean nDCG']:.6f}")
