    ...
    for fraction in downsample_fractions:
        ...
        for iterations in iteration_values:
            for features in feature_values:
                # create model and pipeline for biasedMF
                config_als = BiasedMFConfig(epochs=iterations, embedding_size=features,regularization=0.1,damping=0, user_embeddings=False)
                model_als = BiasedMFScorer(config_als)
                pipe_als = topn_pipeline(model_als)
                
                # run the pipeline with training and validation data
                mean_ndcg = evaluate_with_ndcg(pipe_als, downsampled_train_data, validation_data)
        ...
        final_config = BiasedMFConfig(epochs=best_iterations, embedding_size=best_features,regularization=0.1,damping=0, user_embeddings=False)
        final_model = BiasedMFScorer(final_config)
        ...
        results_downsampling.append({'Fraction': fraction, 'Features': features, 'Iterations': iterations, 'Final Mean nDCG': final_mean_ndcg})
