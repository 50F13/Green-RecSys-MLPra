...
def main():
    ...
    for fraction in downsample_fractions:
        ...
        for k_kandidate in k_values:
            # create model and pipeline for user knn
            config_uknn = UserKNNConfig(max_nbrs=k_kandidate, feedback="explicit")
            model_uknn = UserKNNScorer(config_uknn)
            pipe_uknn = topn_pipeline(model_uknn)
            
            # run the pipeline with training and validation data
            mean_ndcg = evaluate_with_ndcg(pipe_uknn, downsampled_train_data, validation_data)
        ...
        final_config = UserKNNConfig(max_nbrs=best_k, feedback="explicit")
        final_model = UserKNNScorer(final_config)
